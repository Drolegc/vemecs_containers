          ...
          <v-fade-transition>
            <div v-show="!loading2">
              ...
              <v-card-text v-if="!loading2">
                <v-sheet color="rgba(0, 0, 0, .12)">
                  <v-sparkline :value="values2" line-width="2" color="black" height="100%" padding="24" show-labels
                    stroke-linecap="round">
                  </v-sparkline>
                </v-sheet>
              </v-card-text>
...
<script>
  ...
  export default {
    ...
    methods: {
      getData() {
        var socket = io.connect('http://6afa6a1a85b4.ngrok.io', {
          'forceNew': true
        });
        socket.on('messages', (data) => {
          ...
          console.log(this.dataPaciente.nombre == undefined)
      ...
      createPaciente(){
        this.$axios.post('http://localhost:30500/VeMecApi/pacientes/', this.paciente,{ headers: {'Access-Control-Allow-Origin': '*'} })
          .then(()=>{
      ...
      getPaciente() {
        this.$axios.get(`http://localhost:30500/VeMecApi/pacientes/${this.dataPaciente.vemec_id}`,{ headers: {'Access-Control-Allow-Origin': '*'} })
          .then((data)=>{
      ...
      getVemecs(){
        this.$axios.get('http://localhost:30500/VeMecApi/vemecs/',{ headers: {'Access-Control-Allow-Origin': '*'} })
          .then((data)=>{
      ...
      createVemec(){
        this.$axios.post('http://localhost:30500/VeMecApi/vemecs/', this.vemec,{ headers: {'Access-Control-Allow-Origin': '*'} })
          .then(()=>{
            this.getVemecs()
